...
export const fetchContacts = createAsyncThunk<IContactState[], void, {}>(
  ...
  async () => {
    const { data: responseData } = await axiosApi.get<null | IContactsApi>(
      '/contacts.json',
    );
    ...
    if (responseData) {
      fetchedData = Object.keys(responseData).map((id) => ({
        id,
        ...responseData[id],
        photo: responseData[id].photo
          ? responseData[id].photo
          : 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSfHDhRGTty4JUK8TuWxxrGMdy7awUBC4LY0w&s',
      }));
    }
    return fetchedData;
  },
